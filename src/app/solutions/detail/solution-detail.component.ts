...
export class SolutionDetailComponent implements OnInit, AfterViewInit {
    @ViewChild('solutionTitle', { static: false }) solutionTitle: ElementRef<
        HTMLElement
    >;
    @ViewChild('attachmentsModalCloseBtn', { static: false })
    attachmentsModalCloseBtn: ElementRef<HTMLElement>;
    @ViewChild('viewMediaBtn', { static: false }) viewMediaBtn: ElementRef<
        HTMLElement
    >;
    @ViewChild('validationDetails', { static: false })
    validationDetails: ElementRef<HTMLElement>;
    @ViewChild('addValidationDetails', { static: false })
    addValidationDetails: ElementRef<HTMLElement>;
    @ViewChild('editValidationDetails', { static: false })
    editValidationDetails: ElementRef<HTMLElement>;
    @ViewChild('shareBtn', { static: false }) shareBtn: ElementRef<HTMLElement>;
    @ViewChild('discussionModalCloseBtn', { static: false })
    discussionModalCloseBtn: ElementRef<HTMLElement>;
    ...
    ngAfterViewInit() {
        this.showPopularDiscussions('latest');
    ...
    ngOnInit() {
        ...
        this.solutionDataSubcription.subscribe(
            result => {
                if (
                    result.data.solutions.length >= 1 &&
                    result.data.solutions[0].id
                ) {
    ...
    preLoggedInContent(index: number) {
        switch (index) {
            case 0:
                ...
                break;
            case 1:
                this.preLoggedInText = 'to validate this solution.';
                break;
            case 2:
                this.preLoggedInText = 'to collaborate on this solution.';
                break;
    ...
    parseSolution(solution) {
        ...
        if (solution.problems_solutions) {
            ...
            solution.problems_solutions.map(problems => {
                const locations = problems.problem.problem_locations.map(
                    location => {
    ...
    toggleSolutionAttachmentsIndex(dir: boolean) {
        if (
            dir &&
            this.solution_attachments_index <
                this.solution_attachments.length - 1
        ) {
            this.solution_attachments_index++;
            this.solution_attachments_src = this.solution_attachments[
                this.solution_attachments_index
    ...
    onCollaborationSubmit(collaborationData) {
        ...
        this.collaborationService.submitSolutionCollaboratorToDB(
            collaborationData
        );
